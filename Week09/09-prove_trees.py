...
class BST:
    ...
    def _insert(self, data, node):
        ...
        #if data exist in the tree, return none
        if data == node.data:
            return None
        #if data is less than node.data, go left
        elif data < node.data:
            #if left is none, insert data
        ...
        #else the data is greater than node.data, go right
        else:
            #if right is none, insert data
    ...
    def _contains(self, data, node):
        ...
        #else, data is equal to node.data
        else:
            return True
    ...
    def _iter_backward(self):
        """
        Perform a backwards traversal (reverse in order traversal) starting 
        from the root of the BST.  
        """
    ...
    def _traverse_backward(self, node):
        ...
        """
        #if node is not none, traverse backwards
        if node is not None:
            #recursive call
    ...
    def _get_height(self, node):
        ...
        if node is None:
            return 0
        #else, return 1 + the max height of the left or right subtree
        else:
...
def _insert_middle(sorted_list, first, last, bst):
    ...
    #if first is less than or equal to last, insert the middle value
    if first <= last:
    ...
    #else, return none if first is greater than last or if the list is empty   
    else:
        return None
