...
class Translator:
    ...
    def add_word(self, from_word, to_word):
        ...
        """
        #I just changed the variable names to make it easier for me to understand which is which
        englishWord = from_word
        germanWord = to_word
        
        self.words[englishWord] = germanWord
        pass
    ...
    def translate(self, from_word):
        ...
        """
        germanWord = from_word
        
        if germanWord in self.words:
            return self.words[germanWord]
        else:
            return "???"
        pass
...
def summarize_degrees(filename):
    ...
    with open(filename) as file_in:
        for line in file_in:
            #line below is to isolate the education field
            fields = line.split(",")[3]
            # if the field is already in the dictionary, add 1 to the value
            if fields in degrees:
                degrees[fields] += 1
            # if the field is not in the dictionary, add it and set the value to 1
            else:
                degrees[fields] = 1 
...
def is_anagram(word1, word2):
    ...
    #removing spaces and making all letters lowercase so that the words can be compared
    word1 = word1.lower().replace(" ","")
    word2 = word2.lower().replace(" ","")
    
    #for loop to add each letter from word1 and word2 to their respective dictionaries and count the number of times each letter appears
    for letter in word1:
        if letter in checkWord1:
            checkWord1[letter] += 1
        else:
            checkWord1[letter] = 1
            
    for letter in word2:
        if letter in checkWord2:
            checkWord2[letter] += 1
        else:
            checkWord2[letter] = 1
    
    #if the dictionaries are the same, then the words are anagrams
...
class Maze:
    ...
    def move_left(self):
        ...
        """
        #I add the paramater of .curr_x and .curr_y to the map to use the current location on the map.
        if (map[self.curr_x,self.curr_y][0] == False):
    ...
    def move_right(self):
        ...
        """  
        #I add the paramater of .curr_x and .curr_y to the map to use the current location  on the map.
        if (map[self.curr_x,self.curr_y][1] == False):
            print("Can't go that way!")
        else:
            self.curr_x += 1
        pass
    ...
    def move_up(self):
        ...
        """
        #I add the paramater of .curr_x and .curr_y to the map to use the current location on the map.
        if (map[self.curr_x,self.curr_y][2] == False):
            print("Can't go that way!")
        else:
            self.curr_y -= 1
        pass
    ...
    def move_down(self):
        ...
        """
        #I add the paramater of .curr_x and .curr_y to the map to use the current location on the map.
        if (map[self.curr_x,self.curr_y][3] == False):
            print("Can't go that way!")
        else:
            self.curr_y += 1
        pass
...
def earthquake_daily_summary():
    ...
    # ADD YOUR CODE HERE
    #print(data)
    #I created a for loop to the properties key in the data dictionary and then printed the place and mag keys
    for properties in data["features"]:
        print(properties["properties"]["place"] + " - Mag " + str(properties["properties"]["mag"]))
