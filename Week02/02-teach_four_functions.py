...
def calc_factorial(n):
    ...
    O(n) since it only has 1 loop. Linear times.
    """
    factorial = 1
...
def calc_stats(numbers):
    ...
    O(n) + O(n) = O(2n) = O(n) since it has 2 loops. Linear times.
    """
    total_sum = 0
    count = 0
...
def print_triangle(size):
    ...
    O(n^2) since it has 2 loops one of which is nested. Quadratic times.
    """
...
def display_letters_in_names(names):
    ...
    O(n^2) since it has 2 loops one of which is nested. The first loop is the names in the List. Second loop is for every letter in the name. Quadratic times.
    """
    for name in names:
        for letter in name:
            print(letter)
